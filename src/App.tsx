...
function CurrentPath({ fs }: Props) {
  ...
  const fullPath = pwd.map(path => (
    <Button variant="ghost" size="sm" className="text-zinc-600 font-mono text-sm px-1 min-w-5" onClick={changePath.bind(null, path.location)}>
      {path.name}
    </Button>
  ))

  const fullPathAsElements = joinElements(fullPath, <ChevronRight className="text-zinc-400" size={15} />)
